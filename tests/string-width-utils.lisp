...
;; Mirror of lem/common/character/eastasian:*eastasian-full* (Unicode 17.0.0),
;; regenerated by scripts/gen-eastasian.lisp. Every code point in these ranges
;; must be wide (display width 2).
...
(defun str (&rest codes)
  (coerce (mapcar #'code-char codes) 'string))

(deftest unicode-width-correctness
  (testing "combining marks are zero width"
    ;; e + U+0301 COMBINING ACUTE ACCENT renders as é in one column.
    (ok (eql 1 (string-width (str #x65 #x301))))
    ;; U+0301 on its own contributes nothing.
    (ok (eql 0 (string-width (str #x301))))
    ;; U+FE0F VARIATION SELECTOR-16 (Mn) is zero width.
    (ok (eql 0 (string-width (str #xfe0f)))))
  (testing "ZWJ is zero width"
    (ok (eql 0 (string-width (str #x200d)))))
  (testing "modern emoji presentation is width 2"
    ;; U+1F9EA TEST TUBE.
    (ok (eql 2 (string-width (str #x1f9ea))))
    ;; U+1FAE0 MELTING FACE (absent from the pre-Unicode-10 table).
    (ok (eql 2 (string-width (str #x1fae0)))))
  (testing "emoji ZWJ sequence sums component widths"
    ;; Family: man ZWJ woman ZWJ girl ZWJ boy -> four width-2 emoji, three
    ;; zero-width joiners = 8 columns.
    (ok (eql 8 (string-width (str #x1f468 #x200d #x1f469 #x200d
                                  #x1f467 #x200d #x1f466)))))
  (testing "CJK is width 2"
    (ok (eql 4 (string-width (str #x4e16 #x754c)))))
  (testing "ambiguous width honors *ambiguous-character-width*"
    ;; U+00A7 SECTION SIGN is East_Asian_Width Ambiguous.
    (ok (eql 1 (string-width (str #xa7))))
    (let ((*ambiguous-character-width* 2))
      (ok (eql 2 (string-width (str #xa7)))))))
