...
(deftest idle-timer-redisplay-results
  ;; Keep the historical first value (callbacks ran) independent of redisplay.
  (dolist (case '((nil nil t) (nil t t) (t nil nil) (t t t) (t 42 t)))
    (destructuring-bind (on-result result redraw-p) case
      (with-testing-timer-manager ()
        (let ((lem/common/timer::*processed-idle-timer-list* nil)
              (calls 0))
          (start-timer (make-idle-timer (lambda () (incf calls) (values result t))
                                       :name "redraw result" :redraw-on-result-p on-result)
                       1 :repeat t)
          (with-idle-timers ()
            (ok (equal '(nil nil) (multiple-value-list (update-idle-timers))))
            (setf *current-time* 2)
            (ok (equal (list t redraw-p) (multiple-value-list (update-idle-timers))))
            (ok (= calls 1))
            (ok (equal '(nil nil) (multiple-value-list (update-idle-timers)))))))))
  (with-testing-timer-manager ()
    (let ((lem/common/timer::*processed-idle-timer-list* nil)
          (calls 0))
      (dolist (on-result '(t nil t))
        (start-timer (make-idle-timer (lambda () (incf calls) nil)
                                     :name "mixed redraw" :redraw-on-result-p on-result)
                     1))
      (setf *current-time* 2)
      (ok (equal '(t t) (multiple-value-list (update-idle-timers))))
      (ok (= calls 3) "a redraw request must not short-circuit other callbacks"))))
