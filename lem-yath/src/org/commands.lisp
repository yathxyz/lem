         ...
         (multiple-value-bind (indent content-column)
           ...
           (let* ((type (if (org-list-ordered-item-p line)
             ...
             (when (eq type :ordered)
               (setf ordered-p t)))))
        ((or (org-list-line-structural-tab-p line)
             (null (org-navigation-list-anchor line)))
         (setf safe-p nil)
         (return))))
...
(defun org-delete-contains-item-tree-p (start end item)
  "Whether START..END contains ITEM and its complete parsed tree."
  (alexandria:when-let ((tree-end (org-list-item-tree-end item)))
    (and (not (point< item start))
         (not (point< end tree-end)))))
...
(defun org-preflight-ordered-delete (start end)
  ...
  (alexandria:if-let ((anchor (org-delete-ordered-anchor start end)))
    (progn
      (when (and (point< anchor end)
                 (org-list-item-has-direct-body-p anchor)
                 (not (org-delete-contains-item-tree-p start end anchor)))
...
(defun org-repair-ordered-list-at-point ()
  ...
  (multiple-value-bind (lines safe-p ordered-p)
    ...
    (let ((next-by-indent (make-hash-table :test #'eql))
          (changes nil))
      (dolist (line lines)
        (when (org-list-item-line-p line)
