...
| org-agenda / org-super-agenda | ported/partial | `SPC m a` scans the same existing `$WORKDIR`, `$PUBLIC_ORG_DIR`, and public `mcp/` roots as Emacs, using only top-level non-hidden `.org` files; it groups overdue/today/upcoming/unscheduled TODO entries, provides Return/g/q navigation with coalesced asynchronous refreshes, matches Evil-Org agenda `t` with the configured one-key TODO selector, and preserves GNU Org's `C-c C-s`/`C-c C-d` schedule/deadline chords with guarded immediate saves (`src/apps/agenda.lisp`, `scripts/agenda-test.sh`). Active-timestamp events, COMMENT/archive filtering, planning removal/delay-cookie forms, priority/tag/refile/archive mutation, bulk actions, clocks, and the wider Org agenda UI remain gaps. |
