...
| find-name-dired (built-in) | ported/partial | `M-s f` asynchronously fills a persistent, read-only `*Find*` buffer with safely escaped exact-path rows (`src/find-name.lisp`). Dired `m`/`u`/`U`/`t`, `C`/`R`/`D`, marked-or-current targeting, overwrite and top-level recursive copy/delete confirmation, refresh retention, and request-local cancellation are covered in the real TUI; long columns and the wider Dired operation surface remain gaps. |
...
- **Find-name results** persist after opening a match and are safe for spaces,
  ...
  marks plus copy, rename, and confirmed deletion, but remain a compact path
  list without long columns or Dired's broader chmod/chown/link/shell/compress
  command surface.
