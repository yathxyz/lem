...
| org-agenda / org-super-agenda | ported/partial | `SPC m a` scans the same existing `$WORKDIR`, `$PUBLIC_ORG_DIR`, and public `mcp/` roots as Emacs, using only top-level non-hidden `.org` files; it groups overdue/today/upcoming/unscheduled TODO entries plus ordinary, timed, ranged, and repeating active-timestamp events, excludes COMMENT/ARCHIVE trees and non-semantic timestamp contexts, provides Return/g/q navigation with coalesced asynchronous refreshes, matches Evil-Org agenda `t` with the configured one-key TODO selector and `J`/`K` with GNU Org's default A/B/C priority transitions, preserves GNU Org's `C-c C-s`/`C-c C-d` schedule/deadline chords, provides completion-backed, aligned local-tag replacement/clearing on Evil-Org `ct` and GNU Org `C-c C-q`, durably moves a complete subtree to the default adjacent `_archive` file on Evil-Org `dA` (`da` confirms) plus GNU Org's archive chords, and matches the current nil-target GNU `C-c C-w` policy by completing over same-file level-one headings and moving the selected subtree to the target's final-child position. Mutations validate the exact scanned heading and save immediately; refile also rechecks the chosen target, while archive writes the destination before deleting the source and records Org's default context properties (`src/apps/agenda.lisp`, `src/apps/agenda-archive.lisp`, `src/apps/agenda-refile.lisp`, `scripts/agenda-test.sh`). Diary sexps, hour repeaters, full time-grid/range presentation, planning removal/delay-cookie forms, configurable/cross-file refile targets and creation/copy/reverse or prefix/cache variants, custom archive destinations/local archive sibling or tag commands, bulk actions, clocks, and the wider Org agenda UI remain gaps. |
