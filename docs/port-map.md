...
| orderless | ported/partial | ordinary-buffer completion has escaped-space components, whole-query smart case, any-order literal/regexp filtering, and `~ = ^ ! ,` affix dispatch through `M-Space`; CL-PPCRE differs from Emacs regexp syntax, and `%` char-fold plus `&` annotation dispatch remain gaps (`src/orderless.lisp`, `src/auto-completion.lisp`, `patches/lem-completion-lifecycle.patch`, `scripts/orderless-completion-test.sh`) |
| marginalia | ported/partial | `src/annotations.lisp` adds bounded display-only metadata to the common prompt categories: M-x shows active bindings and command documentation; ordinary/project buffers show state, size, mode, and path; ordinary/recent/project files show local modes, size, age/date, and conditional owner. Upstream prompt items retain identity with only their detail changed; custom project/recent items preserve raw values behind injective display labels and prompt-correct ranges (`patches/lem-completion-detail-accessor.patch`; common-case TUI coverage in `scripts/completion-test.sh`, `scripts/prompt-completion-test.sh`, `scripts/project-navigation-test.sh`, and `scripts/daily-workflows-test.sh`). Richer variables/functions/faces/packages/libraries/themes/bookmarks categories, dynamic alignment, and remote fields remain gaps. |
