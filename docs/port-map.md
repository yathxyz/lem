...
| expreg | ported/partial | repeated `SPC v` expands lexical subword → symbol → Python/JSON syntax nodes, with innermost delimiter-interior and enclosing-pair tiers plus a paragraph fallback in parserless modes (`src/vi.lisp`, `scripts/expreg-test.sh`). `M-x expreg-contract`, declared but unbound in the Emacs configuration, walks backward through the generated buffer-local sequence; `SPC v` then moves forward again. Parser results and contraction state are valid only for the unchanged buffer. Other languages retain textual fallback; parsed strings deliberately do not expose balanced-delimiter tiers inside their contents, malformed error fragments use useful containing nodes rather than Expreg's partial error ranges, and arbitrary pre-existing Visual selections are not invented as contraction tiers. |
