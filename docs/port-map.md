...
| marginalia | ported/partial | `src/annotations.lisp` adds bounded display-only metadata to the common prompt categories: M-x shows active bindings and command documentation; `src/help.lisp` shows callable type/signature/docs and variable type/bounded or censored value/docs; ordinary/project buffers show state, size, mode, and path; ordinary/recent/project files show local modes, size, age/date, and conditional owner. Exact package-qualified identity and secret censoring are TUI-tested. Richer faces/packages/libraries/themes fields, bookmark context, dynamic alignment, and remote fields remain gaps. |
| consult | ported/partial | `src/project.lisp` supplies persistent project MRU, tracked+untracked annotated file selection, bounded asynchronous regexp search, and Emacs-style switch dispatch on `SPC p f/g/p`. `src/project-picker.lisp` makes `SPC SPC` a grouped Project Buffer/File/Root picker with fixed source order, `b/f/r Space` narrowing, empty-Backspace widening, group rotation, identity-preserving duplicate labels, and reversible preview-on-move. The switch menu preserves `f/g/d/v/e/o`: `v` opens Git through Legit at the selected root, while `e` uses Lem's rooted terminal and `o` uses rooted M-x-style command execution, approximations of `project-eshell` and `project-any-command`. Unopened-file previews deliberately remain raw UTF-8 text, avoid file/mode/switch/kill hooks, and skip undecodable, binary, or over-1-MiB files because generic Lem major-mode activation is not isolated from parser and hook side effects; ordinary window-display hooks can still run. |
...
| csharp-mode / csharp-ts-mode | ported/partial | `.cs` and `.csx` files use a native C# mode with C-like indentation and a bounded TextMate-style tokenizer, then automatically start packaged **csharp-ls** at the nearest `.sln`, `.csproj`, or `.git` root. The installed-wrapper gate obtains a real semantic error via pull diagnostics. This does not reproduce csharp-ts tree-sitter parsing, progress UI, or filesystem-watch notifications. |
...
| expreg | ported/partial | repeated `SPC v` expands lexical subword → symbol → Python/JSON syntax nodes, with innermost delimiter-interior and enclosing-pair tiers plus a paragraph fallback in parserless modes (`src/vi.lisp`, `scripts/expreg-test.sh`). The parser result is cached as immutable byte ranges for one unchanged expansion sequence. Other languages retain textual fallback; contraction is unbound, parsed strings deliberately do not expose balanced-delimiter tiers inside their contents, and malformed error fragments use useful containing nodes rather than Expreg's partial error ranges. |
...
| rainbow-delimiters | ported/partial | `src/ui.lisp` colors every syntax-table delimiter pair by depth in programming buffers, excluding strings and comments, and `src/theme.lisp` maps all nine default depths to the active Modus palette. Show-paren remains active. Unlike Emacs rainbow-delimiters, unmatched or mismatched closers do not receive a dedicated error face, and individual Lem syntax tables can classify delimiters differently (`scripts/ui-parity-test.sh`). |
...
| ibuffer | ported/partial | `C-x C-b` keeps Lem's native marked multi-column chooser but partitions its recency-ordered snapshot with the configured exclusive first-match org/tramp/emacs/ediff/dired/terminal/help groups, omits empty groups, and places unmatched buffers under `Default`. Group labels remain visible during fuzzy narrowing; Return, mark/save/kill, control-character-safe labels, and reload are retained (`src/buffer-list.lisp`, `scripts/buffer-list-test.sh`). Unlike Ibuffer, groups are first-row column labels rather than collapsible heading rows, and its wider sorting/filter/format command surface is absent. |
...
- **Buffer list UI**: `C-x C-b` now has the exact configured first-match group
  partition and empty-group policy, but uses first-row group column labels
  rather than collapsible Ibuffer headings and does not reproduce Ibuffer's
  wider interactive sorting, filtering, and format controls.
