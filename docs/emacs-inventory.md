...
| **YAML / Meson / nginx** | `yaml-mode`, `meson-mode`, `nginx-mode` | declared in nix, **no explicit config (defaults)** | — | — | — | Lem covers the packages' effective default file associations and highlighting; Meson additionally reproduces its pinned completion tables, Eldoc signatures, error locations, and representative SMIE indentation rules |
