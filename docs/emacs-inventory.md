...
- **Org capture + org-roam + dailies + journal** (`SPC o`, `SPC n r *`, `SPC n j j`) — Lem now has bounded native Org editing, in-buffer scheduling/deadline insertion, replacement and removal on the stock chords, metadata-aware Org/Markdown roam-node selection, the configured five roam capture templates with finalize/abort and deferred insertion, daily, journal, general Org capture, and agenda implementations. Org-roam database/backlink/autosync semantics, the full calendar/timestamp surface, and the full general capture/journal interfaces remain gaps.
