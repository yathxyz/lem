...
| Remaining Evil-Org / evil-collection integrations | The native Org subset now provides the configured endpoints, insert/append commands, active object/element/greater/subtree text objects, and bounded `d/x/X/< />` repair. Richer destructive repair, unsupported Org syntax, region-aware Meta/list/table edge cases, source editing, timestamp/schedule/deadline workflows, and integrations for every other Emacs mode remain absent. |
