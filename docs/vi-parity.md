...
| Remaining Evil-Org / evil-collection integrations | The native Org subset now provides the configured endpoints, sentence/structural-paragraph motions, insert/append commands, active object/element/greater/subtree text objects, and bounded `d/x/X/< />` repair. Full non-heading `gh/gl/gk/gj` traversal, richer destructive repair, unsupported Org syntax, region-aware Meta/list/table edge cases, source editing, timestamp/schedule/deadline workflows, and integrations for every other Emacs mode remain absent. |
