...
| Remaining Evil-Org / evil-collection integrations | The native Org subset now provides the configured endpoints, insert/append commands, and active object/element/greater/subtree text objects, but not structural `d/x/X`, richer unsupported Org syntax, the full list/table meta theme, source editing, timestamp/schedule/deadline workflows, or integrations for every other Emacs mode. |
