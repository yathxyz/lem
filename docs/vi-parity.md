...
| evil-surround | `ys{motion}`, `ds{char}`, `cs{old}{new}`, visual `S`; padded `(`/`[`/`{` and compact closing-delimiter variants. Delete/change selects the narrowest balanced character pair, isolates code/string/comment/fence syntax, ignores escapes and decoys, preflights read-only boundaries, fails closed on malformed or triple-quoted input, and remains one undo step | `surround-test.sh`; interactive check 10; Org operator compatibility gate |
...
| Region expansion | Repeated `SPC v` expands lexical subwords and symbols through Python/JSON syntax nodes, including string and delimiter boundaries; parserless modes retain balanced-delimiter and paragraph fallback | `expreg-test.sh`; interactive check 21 |
...
| Full Avy dispatch and exact presentation | Navigation and label selection are implemented, but the default `x/X/t/m/n/y/Y/i/z` actions and `?` help are absent; exotic display/syntax geometry and exact Emacs minibuffer presentation remain approximate. |
| Full expreg language coverage and contraction | Parser-backed expansion currently covers exact Python and JSON file modes. Other languages use lexical, balanced-delimiter, and paragraph fallback; contraction is unbound. Parsed strings prioritize syntax boundaries over Expreg's balanced-delimiter tiers inside string contents, and malformed ERROR fragments use useful containing nodes rather than partial error ranges. |
| Full evil-surround grammar | Common delimiters, padding, and syntax-aware balanced character matching are present; tag/function prompts and multi-character block-string editing are not. |
