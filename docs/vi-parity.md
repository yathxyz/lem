...
| Region expansion | Repeated `SPC v` expands lexical subwords and symbols through Python/JSON syntax nodes, including string and delimiter boundaries; parserless modes retain balanced-delimiter and paragraph fallback. The configured but unbound `M-x expreg-contract` walks backward through the unchanged generated sequence, and `SPC v` can expand forward again. | `expreg-test.sh`; interactive check 21 |
...
| Full expreg language coverage | Parser-backed expansion currently covers exact Python and JSON file modes. Other languages use lexical, balanced-delimiter, and paragraph fallback. Parsed strings prioritize syntax boundaries over Expreg's balanced-delimiter tiers inside string contents, malformed ERROR fragments use useful containing nodes rather than partial error ranges, and arbitrary pre-existing Visual selections are not retained as contraction tiers unless Lem generated them. |
