  ...
  (sdl2:with-event-loop (:method :wait)
    ...
    (:mousebuttondown (:timestamp timestamp :button button :x x :y y :clicks clicks)
     (multiple-value-bind (button x y clicks)
         (corrected-mouse-button-fields sdl2-ffi:+sdl-mousebuttondown+
                                        timestamp button x y clicks)
       (on-mouse-button-down display button x y clicks)))
    (:mousebuttonup (:timestamp timestamp :button button :x x :y y)
     (multiple-value-bind (button x y clicks)
         (corrected-mouse-button-fields sdl2-ffi:+sdl-mousebuttonup+
                                        timestamp button x y 1)
