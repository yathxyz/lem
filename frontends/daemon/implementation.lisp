...
(defun overlay-cell-run (target column source start end)
  "Copy disjoint one-column cells, repairing wide glyphs at the clipped edges."
  (let* ((cells (cell-row-cells target))
         (first (max 0 column))
         (last (min (length cells) (+ column (- end start)))))
    (when (< first last)
      (clear-cell-at target first)
      (clear-cell-at target (1- last))
      (let ((source-start (+ start (- first column))))
        (replace cells (cell-row-cells source)
                 :start1 first :end1 last :start2 source-start)
        (replace (cell-row-faces target) (cell-row-faces source)
                 :start1 first :end1 last :start2 source-start)))))

(defun overlay-cells (target column source)
  (let* ((cells (cell-row-cells source))
         (faces (cell-row-faces source))
         (length (min (length cells) (length faces)))
         (end length))
    (declare (type fixnum end))
    ;; Blank padding dominates many view rows. Inspect only the suffix so
    ;; occupied cells do not pay a run-detection cost on every iteration.
    (when (and (> length 1)
               (not (eq cells (cell-row-cells target)))
               (not (eq faces (cell-row-faces target))))
      (let ((last (svref cells (1- length))))
        (when (and (stringp last) (= 1 (length last))
                   (char= #\Space (char last 0))
                   (= 1 (char-width #\Space 0))
                   (eq last (svref cells (- length 2))))
          (loop :while (and (plusp end) (eq last (svref cells (1- end))))
                :do (decf end)))))
    (loop :for index :from 0 :below end
          :for cell := (svref cells index)
          :for face := (svref faces index)
    ...
    (when (< end length)
      (overlay-cell-run target column source end length)))
  target)
