...
(defun last-mouse-event () (frame-last-mouse-event (current-frame)))
(defun set-last-mouse-event (mouse-event)
  (setf (frame-last-mouse-event (current-frame)) mouse-event))
...
(defmethod handle-button-1 ((separator window-separator) x y clicks)
  (setf (frame-dragged-separator (current-frame)) separator))
...
(defmethod handle-mouse-event ((mouse-event mouse-button-up))
  (setf (frame-dragged-separator (current-frame)) nil)
...
(defun handle-mouse-hover-overlay (window point)
  (multiple-value-bind (overlay callback)
    ...
    (when (and overlay
               (not (eq overlay (frame-hover-overlay (current-frame)))))
      (setf (frame-hover-overlay (current-frame)) overlay)
      (funcall callback window point)
      (return-from handle-mouse-hover-overlay))
    (unless overlay
      (when (frame-hover-overlay (current-frame))
        (alexandria:when-let (callback (overlay-get (frame-hover-overlay (current-frame)) :unhover-callback))
          (funcall callback window point))
        (setf (frame-hover-overlay (current-frame)) nil)))))
...
(defmethod handle-mouse-event ((mouse-event mouse-motion))
  (let ((separator (frame-dragged-separator (current-frame))))
    (cond ((null separator)
           (multiple-value-bind (window x y)
               (focus-window-position (current-frame)
                                      (mouse-event-x mouse-event)
             ...
             (when window
               (handle-mouse-hover (window-buffer window)
                                   ...
                                   :x x
                                   :y y))))
          ((typep separator 'window-vertical-separator)
           (let ((x (mouse-event-x mouse-event))
                 (button (mouse-event-button mouse-event)))
             (when (eq button :button-1)
               (let ((diff-x (- x (window-separator-start-x separator))))
                 (unless (zerop diff-x)
                   (when (if (plusp diff-x)
                             (shrink-window-width
                              (window-separator-grabbed-window separator)
                              diff-x)
                             (grow-window-width
                              (window-separator-grabbed-window separator)
                              (- diff-x)))
                     (setf (window-separator-start-x separator) x)))))))
          ((typep separator 'window-horizontal-separator)
           (let ((y (mouse-event-y mouse-event))
                 (button (mouse-event-button mouse-event)))
             (when (eq button :button-1)
               (let ((diff-y (- (window-separator-start-y separator) y)))
                 (unless (zerop diff-y)
                   (when (if (minusp diff-y)
                             ...
                             (grow-window-height
                              (window-separator-grabbed-window separator)
                              diff-y))
                     (setf (window-separator-start-y separator) y)))))))
          ((typep separator 'leftside-window-separator)
           (let ((x (mouse-event-x mouse-event))
                 (button (mouse-event-button mouse-event)))
             (when (eq button :button-1)
               (let ((diff-x (- x (window-separator-start-x separator))))
                 (unless (zerop diff-x)
                   (when (resize-leftside-window-relative diff-x)
                     (setf (window-separator-start-x separator) x)))))))
          ((typep separator 'rightside-window-separator)
           (let ((x (mouse-event-x mouse-event))
                 (button (mouse-event-button mouse-event)))
             (when (eq button :button-1)
               (let ((diff-x (- x (window-separator-start-x separator))))
