...
(defun call-with-modify-buffer (point n function)
  (without-interrupts
    (let ((buffer (point-buffer point)))
      ...
      ;; This scope performs one insertion or deletion, without change hooks.
      ;; A changed line count detects newline edits; otherwise only the source
...
(defmethod insert-string/point :around (point string)
  ...
  (let* ((buffer (point-buffer point))
         (position (and (not (undo-tree-replaying-buffer-p buffer))
                        (or (buffer-%enable-undo-p buffer) (inhibit-undo-p))
                        (position-at-point point))))
    (when (buffer-%enable-undo-p buffer)
      (ensure-buffer-undo-tree buffer))
    (flet ((record (result)
             (when (and position (plusp (length result)))
...
(defmethod delete-char/point :around (point remaining-deletions)
  ...
  (let* ((buffer (point-buffer point))
    ...
    (when (buffer-%enable-undo-p buffer)
      (ensure-buffer-undo-tree buffer))
    (flet ((record (string)
             (when (and position (plusp (length string)))
