...
(defun dap-test-gutter-contract ()
  (let* ((*dap-breakpoints* (make-hash-table :test #'equal))
         (*dap-session* nil)
         (buffer *dap-test-buffer*)
         (path (dap-buffer-path buffer))
         (original-path (symbol-function 'dap-buffer-path))
         (path-calls 0))
    (unwind-protect
         (with-point ((point (buffer-start-point buffer)))
           (setf (symbol-function 'dap-buffer-path)
                 (lambda (&optional (target (current-buffer)))
                   (incf path-calls)
                   (funcall original-path target)))
           (flet ((check-gutter (glyph calls label)
                    (setf path-calls 0)
                    (let ((content (dap-gutter-content buffer point)))
                      (dap-test-check
                       (and (= calls path-calls)
                            (if glyph
                                (and content
                                     (string= glyph
                                ...
                                (null content)))
                       label))))
             (check-gutter nil 0 "idle-debugger-gutter-does-not-resolve-path")
             (setf path-calls 0)
             (dap-attach-breakpoints-to-buffer buffer)
             (dap-test-check (zerop path-calls)
                             "empty-breakpoint-attachment-does-not-resolve-path")
             (setf *dap-session* (%make-dap-session :state :running))
             (check-gutter nil 0 "running-debugger-without-markers-skips-path")
             ;; A stopped location must render even with no source breakpoints.
             (setf (dap-session-stopped-path *dap-session*) path
                   (dap-session-stopped-line *dap-session*) 1)
             (check-gutter "▶" 1 "stopped-line-renders-without-breakpoints")
             (setf (dap-session-stopped-line *dap-session*) 2)
             (let ((breakpoint (make-dap-breakpoint :path path :line 1)))
               (dap-store-breakpoint breakpoint)
               (dap-attach-breakpoints-to-buffer buffer)
               (dap-test-check (dap-breakpoint-point breakpoint)
                               "nonempty-breakpoints-still-attach")
               (check-gutter "○" 1 "pending-gutter-resolves-path-once")
               (setf (dap-breakpoint-verified-p breakpoint) t)
               (check-gutter "●" 1 "verified-gutter-resolves-path-once")
               (setf (dap-session-stopped-line *dap-session*) 1)
               (check-gutter "▶" 1 "stopped-line-takes-priority-over-breakpoint")
               (line-offset point 2)
               (check-gutter nil 1 "other-lines-have-no-debugger-marker")
               (dap-remove-breakpoint breakpoint))
             (setf *dap-session* nil)
             (check-gutter nil 0 "last-breakpoint-removal-restores-idle-path")))
      (setf (symbol-function 'dap-buffer-path) original-path)
      (maphash (lambda (filename breakpoints)
