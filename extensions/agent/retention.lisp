...
Malformed JSON names are included but do not count toward admission; no payload is read."
  ...
  (bt2:with-lock-held ((manager-storage-lock manager))
    (bt2:with-lock-held ((manager-lock manager)) (check-manager-open manager))
    (let ((names nil) (total 0))
      (labels ((include (id)
                 (when (and (or (null after) (string> id after)) (not (member id names :test #'equal)))
                   (push (copy-seq id) names)
                   (setf names (sort names #'string<))
                   (when (> (length names) limit)
                     (setf (cdr (nthcdr (1- limit) names)) nil)))))
        (setf total (store:map-private-json (manager-directory manager)
                                            (lambda (id path) (declare (ignore path)) (include id))))
        ;; An initial write or deletion may have failed with no remaining file.
        ;; Keep those reserved slots addressable without an unbounded catalog.
        (bt2:with-lock-held ((manager-lock manager))
          (maphash (lambda (id session) (declare (ignore session)) (include id)) (manager-session-table manager))
          (when (manager-pending-discard manager) (include (first (manager-pending-discard manager))))))
...
Reading never attaches a session, invokes tools, opens editor files or checkpoints."
  ...
  (bt2:with-lock-held ((manager-storage-lock manager))
    ...
    (let* ((pending (bt2:with-lock-held ((manager-lock manager)) (manager-pending-discard manager)))
      ...
      (if (null (store::path-stat (store::record-path directory id)))
          (cond ((equal id (first pending))
                ...
                ((find-session manager id)
                 (values nil (copy-seq "missing") "This reserved session has no durable journal. Stored contents and counts are unknown."))
                (t (error "No session journal exists for this ID")))
...
Errors, including directory fsync failure after unlink, retain the capacity slot."
  (unless (and (valid-id-p id)
               (or (equal expected-fingerprint "missing") (text-p expected-fingerprint 64 64)))
    ...
    (let ((session nil) (pending nil))
      ...
      (when session
        (bt2:with-lock-held ((session-lock session))
          (unless (and (session-closed session) (session-close-pending session))
      ...
      (let* ((directory (store:ensure-private-directory (manager-directory manager) :create nil))
        ...
        (if (and absent (or pending (and session (equal "missing" expected-fingerprint))))
            ...
            (progn
              (unless (or pending acknowledge-uncertain)
                (error "Explicitly acknowledge missing journal contents before releasing this slot"))
              (bt2:with-lock-held ((manager-lock manager))
            ...
            (progn
              (multiple-value-bind (record fingerprint diagnostic) (read-session-journal manager id)
                ...
                (when (and (or (uncertain-session-history-p record) (and session (session-poisoned session)))
                           (not acknowledge-uncertain))
